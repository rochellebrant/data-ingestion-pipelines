...
// dbutils.widgets.text("jobGroup","538")
...
var collection: org.apache.spark.sql.DataFrame = _
...
try {
  ...
  for (row <- collection.rdd.collect) {    
    val nbData = NotebookData(
      "Tables",
      3600000,
      Map(
        "pkTblJobQueue" -> row.getAs("pkTblJobQueue").toString(),
        "jobGroup" -> jobGroup.toString(),
        "jobOrder" -> jobOrder.toString(),
        "jobNum" -> row.getAs("jobNum").toString(),
        "jobStepNum" -> row.getAs("jobStepNum").toString(),
        "runID" -> runID,
        "pipelineName" -> pipelineName,
        "dataFactoryName" -> dataFactoryName,
        "keyVaultName" -> row.getAs("keyVaultName").toString(),
        "failureNotificationEmailIDs" -> row.getAs("failureNotificationEmailIDs").toString()
      )
    )
...
resultant= Await.result(res, 380000 seconds) // Blocking call
